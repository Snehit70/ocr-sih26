...
export default function LandingHeader() {
  ...
  return (
    <header className="sticky top-0 z-50">
      <a
        href="#product"
        className="sr-only focus:not-sr-only focus:absolute focus:left-3 focus:top-3 focus:z-[60] focus:rounded-md focus:bg-white focus:px-3 focus:py-2 focus:text-sm focus:font-semibold focus:text-slate-900"
      >
        Skip to content
      </a>
      ...
      <div className="border-b border-slate-200/80 bg-white/95 backdrop-blur">
        <div className="mx-auto flex h-[4.25rem] max-w-6xl items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-2">
            <Link
              href="/scan"
              className="hidden h-10 items-center gap-1.5 rounded-lg bg-[#1D4ED8] px-4 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-[#1E40AF] sm:inline-flex"
            >
              Start an inspection
        ...
        {open ? (
          ...
          >
            ...
            <Link
              href="/scan"
              onClick={() => setOpen(false)}
              className="mt-2 block rounded-lg bg-[#1D4ED8] px-3 py-2.5 text-center text-sm font-semibold text-white sm:hidden"
            >
              Start an inspection
            </Link>
          </nav>
